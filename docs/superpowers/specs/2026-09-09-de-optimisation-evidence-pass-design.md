...
Brief date 2026-09-09 · Author: Claude (Fable 5.1) working from the repo, `dist/`, and the data files · Status: **Strategy A APPROVED by the breeder 2026-09-09**; rule numbers corrected per the breeder (§7); review-name corrections pending (§8). Nothing on the site has been edited yet.
...
Six house rules actively manufacture the pattern ChatGPT is describing. They are in `docs/reference/WORKFLOW.md` Sprint 4 and the keyword-verifier agent:
...
### Does Strategy A fix each confirmed finding?

| Finding | Fixed by A? | How, and what else it needs |
|---|---|---|
| Concept repetition | **Yes** | term budgets per page type, enforced by `evidence_audit.py`; word count falls as a by-product |
| Homepage does too many jobs | **Partly** | A removes what each section repeats, which is most of the 8,830 words; it does not reduce the 25 jobs because the breeder ruled the sections stay. Once the rail and hero stop describing the page as "everything about C.A.Gs", the remaining length reads as depth, not sprawl |
| Scam / legit propagation | **Yes** | the budget line "scam ≤2, legit 0 outside the two owning pages" is the Strategy B routing folded into A; 37 and 32 pages get one sentence + link |
| Engineered titles | **Yes, via the rule change** | the skill only checks length; the fix is retiring "≤205, never short" and rewriting titles to ≤70 chars, one clause |
| Claims outrun proof | **Yes when proof objects arrive; partly until then** | today each credential is said once and links to the trust section; the proof object (redacted USDA record, example DNA / vet / CITES certificate) replaces the assertion when the breeder supplies it. NOT FETCHED until then |
| Fact / observation / recommendation blended | **Yes** | the statement-label component, required wherever species, health or comparison claims appear |
| Do not answer with more pages | **Yes** | A adds zero pages; C is deferred |
...
| Rule 57 "150+ entity mentions" | **Breeder's correction:** the target is 95–105. Recommended reading (Recommended): 95–105 **distinct** entities present on the page, each said once where load-bearing; counting *mentions* would re-create the floor that caused the repetition. Trade-off: distinct-entity counting needs the entity catalog as the reference list, which `cag-entity-agent` already is |
| Rule 56 "150–200 keyword variants documented" | **Breeder's correction:** per page, count the variants the top competitor page for that keyword actually uses (fetched in Sprint 0, never assumed), then target that count +5 to +10. The fixed 150–200 was the error from the start |
...
- **Review attribution (breeder: the reviews are real, a name is mixed up).** Three quotes carry five names, and the mix-up is not only on the homepage: the reviews hub `/african-grey-reviews/` lists each quote under both names, and the same quotes appear on 12 pages in total. The breeder supplies the correct name for each quote once; the fix then sweeps every file listed.

| Quote (opening words) | Credited to | Where |
|---|---|---|
| Q1 "I searched for African Grey parrots for sale near me for months before finding C.A.Gs…" | **Clifford Hutter**, Secaucus NJ | homepage feature block, reviews hub |
| Q1 (same text) | **Archie Obrien**, Farmingdale NY (photo on file) | homepage bottom grid, reviews hub |
| Q1 (same text, name to confirm) | — | eggs, pros-and-cons, congo-for-sale, congo-vs-timneh, timneh-for-sale |
| Q2 "At first I was hesitant about buying a bird online, but C.A.Gs made the process stress-free…" | **Richard Woodard**, Winter Haven FL (photo on file) | homepage ×2, reviews hub, trusted-breeders |
| Q2 (same text) | **Archie Obrien** | eggs, pros-and-cons, congo-for-sale, congo-vs-timneh, timneh-for-sale |
| Q3 "I ordered a Congo African Grey from C.A.Gs and the experience was flawless…" | **Albert Schroder**, Santa Clara CA | homepage feature block, reviews hub |
| Q3 (same text) | **Catherine Kempf**, Schaumburg IL (photo on file) | homepage bottom grid, reviews hub, trusted-breeders |
| Q3 (same text, name to confirm) | — | vs-amazon, vs-cockatoo, male-vs-female ×2 |

Needed from the breeder: for Q1, Q2 and Q3, the one real name each; and for the names left over, the review they actually wrote, or confirmation that they are removed. Until then the review sections ship unchanged; the `review-attribution-unique` check will fail on them by design.
...
**Answered 2026-09-09:** Strategy A approved. Remaining input needed: the three review names above.

## 9. What Happens Next (Approved)
