...
- `scripts/page_hardening_scan.py` — static half of the `cag-page-hardening` skill: scans `dist/` for the defects source review can't see (`header-not-title-case`, `img-no-srcset`, `opacity-dims-text-contrast`, `clay-small-text-contrast`, `absolute-hero-not-unwound`, `smooth-scroll-breaks-anchors`). Run BEFORE `cag-final-page-pass` on every page; pair it with the skill's runtime browser probes (overflow/contrast/sizing need a real viewport).
