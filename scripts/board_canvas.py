...
def file_token(candidate):
    ...
    cutter's split, and the design-canvas helper refuses `+`, so a refresh id travels as
    `_` on every surface outside the record and build_page_board.py maps it back.
    Unambiguous by construction: ids are ^[a-z0-9-]+(#[a-z0-9-]+)?$, so a `_` on disk can
    only ever have been the one `#`."""
