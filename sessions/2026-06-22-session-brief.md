...
**UPDATE (2026-06-26):** Deeper scan found the issue is site-wide, not just one line — across the whole site there are only **3 distinct testimonial texts** rotated across **6 names** (the "top-notch/seamless" text is on BOTH `Clifford Hutter` (index.astro `reviews[]`) and `Archie Obrien` (index.astro `bottomReviews[]`); "stress-free" → `Richard Woodard` ×2; "flawless" → `Albert Schroder` + `Catherine Kempf`). Same words / different people = fabrication fingerprint. `data/case-studies.json` is EMPTY (its own rule: entries must come from Mark or Teri directly). Bird pages carry **no Review/AggregateRating schema** (only index.astro Product schema has `reviewCount:52` — untouched). **RESOLVED 2026-06-26 — removed, did not fabricate.** Breeder first asked for the quotes to be generated; I declined (writing named buyer testimonials = fabricated reviews, against `case-studies.json`'s own rule + FTC consumer-review rule + the same reason the interior batch *deleted* 5 fake testimonials). Breeder then approved the honest fix: **deleted the entire byte-identical "bottom reviews" section** (`#reviews-bottom` — the Archie Obrien / Catherine Kempf / Richard Woodard rotated-index trio) from all 6 `/available/` pages, removed its TOC entry, and renumbered each TOC to 1..21. Kept the individuated MID review set (Lawrence Brunner / Sandra Soliz / Ida Brim). Build clean; `final_page_audit.py --birds` = 6 PASS-WITH-WARNINGS, heading gate intact (H5:5 / H6:5), no Review/AggregateRating schema on any bird page. **STILL OPEN for Mark & Teri:** the MID set + the homepage `reviews[]`/`bottomReviews[]` arrays are also invented (3 texts / 6 names) and the `reviewCount:52` should be reconciled to genuinely-collected reviews — collect REAL buyer quotes (Google/email, with consent) via the `cag-review-collection-agent` pattern, log to `case-studies.json`, then we can reinstate honest per-page testimonials.
