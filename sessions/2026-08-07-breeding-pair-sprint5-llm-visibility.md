...
- **BREEDER QUESTION — is there a real story behind the inverted price ladder?** Raised by the non-commodity pass 2026-08-08. Specifically: the first time a buyer pushed back on paying $5,500 for the one-clutch pair, or the decision to price Sally & Odin above Talker & Jane. The Key Takeaway is factually unimpeachable and still writable by any model handed the pairs table, because its job is summary. A real moment behind the ladder is the one thing that would make it unwritable by anyone else. Nothing is blocked on this — it raises a ceiling, it does not fix a defect.
- **The Key Takeaway's causal claim was challenged and the challenge did NOT hold — do not "fix" it.** The non-commodity pass argued the takeaway makes *clutch count* the price driver while the real policy is *age*, and called it falsifiable. Checked against `data/price-matrix.json`, whose own `notes` read: *"Price is INVERSE to age: fewer clutches costs more because the pair has more productive years ahead."* The breeder's note uses both framings in one sentence, and the page does the same — *"the price rises as the clutch count falls, because you are buying the breeding years ahead."* The causal clause already names the true driver. The page mirrors the data file; there is nothing to correct.
- **Harness gap worth charging to the tooling:** the same pass drafted a takeaway whose opener collided with a live congo-pair sentence at **8 words**. `dup_content_audit.py` uses ≥12-word shingles, so it would have passed that through. Sibling crossover shorter than 12 words is currently invisible to the gate. Not a page defect — a known blind spot to weigh before the next for-sale build.
