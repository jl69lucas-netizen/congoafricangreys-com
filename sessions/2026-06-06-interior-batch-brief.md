...
- **[NEEDS USER DECISION — privacy-policy, 2026-06-11] Legal text names "Zelle or Cash App" as payment methods.** The contact-form rule says never hardcode a payment processor (`[PAYMENT_METHOD_TBD]`), but the Non-Negotiable "same content" rule forbids rewriting legal prose, so we left the privacy-policy text verbatim (the inquiry form itself names NO processor — compliant). **Recommended:** keep as-is — a privacy policy *describing* what data is collected (incl. which payment channels) is factual disclosure, not a sales-page processor promise; trade-off: if Zelle/Cash App is no longer accurate, the legal text itself is stale and Mark & Teri should supply corrected wording (we won't rewrite legal copy unprompted).
...
- 2026-06-11: **Phase 7 / Cluster E complete (7.1 contact-us + 7.2 privacy-policy).** contact-us: ContactPage schema added (+ existing LocalBusiness/PetStore via Schema.astro), `generate_lead` made `is:inline` (verified ×1 in dist), `hideGlobalCta` passed (form = conversion point), hero claim aligned to confirmed 24-hour stat, meta rewritten with `| C.A.Gs – Midland, TX` suffix (title 142 / desc 278). privacy-policy: visual shell only — minimal dark hero, 760px prose container (was legacy max-w-4xl), WebPage schema, `hideGlobalCta` (page has own CTA block), meta 79/270; legal prose verified byte-identical vs HEAD. Both: 0 escaped svg, 0 emoji, absolute canonicals, 1 BreadcrumbList each, indexable. NOT committed (orchestrator verifies + commits). Open Flag logged re: Zelle/Cash App in legal text.
...
- 2026-06-11: **PLAN COMPLETE — all 18 tasks (Clusters A–E) + Phase 8 finalize.** 6.1 price pillar (11771f0, AggregateOffer 1500–3500 + 6 per-bird Offers, every figure data-traced, invented competitor prices removed). 7.1 contact-us (ContactPage schema, GA4 generate_lead made is:inline, hideGlobalCta) + 7.2 privacy-policy (shell only, legal text byte-identical) (7b84f87). Sitemaps regenerated 100 URLs / 0 phantoms (5e49abc). Health sweep PASS. All 18 slugs live-verified 200 (note: health-guarantee real slug = african-grey-parrot-health-guarantee; /health-guarantee/ is a 301 alias). IndexNow 200 for all 18 URLs. CLAUDE.md Active Session updated; memory written. Outstanding: 3 Open Flags above (pellet brand / 127-review count / privacy Zelle-CashApp wording) — all need breeder input, none block anything.
- 2026-06-11: **All 3 Open Flags RESOLVED by breeder.** ① Pellet brand: no single house brand — endorse the 3–5 best reviewed brands; best-food FAQ now states first-person that every Grey is weaned onto one of Harrison's / Roudybush / TOP's / Zupreem Natural. ② AggregateRating reviewCount corrected **127 → 52 (real count)** site-wide: homepage ×3, african-grey-reviews (AGG_COUNT + meta desc), trusted-breeders ×1; rating stays 4.9. The scams page "+127%" is an unrelated chart stat — untouched. ③ Privacy policy: "Zelle or Cash App" removed per breeder instruction → "the payment method confirmed with you during your reservation" (consistent with [PAYMENT_METHOD_TBD]).
