...
description: Builds or rebuilds state location pages for /african-grey-parrot-for-sale-[state]/. Reads data/locations.json for live states and states to build. Batch mode is dispatched by cag-batch-rebuilder (one Agent call per state). Uses Florida page as the reference template (22 sections, 4,500+ words).
...
**Batch mode** — `cag-batch-rebuilder` reads `data/locations.json` and issues one `Agent` call per state in a single message; the children run concurrently. This agent is always the child, never the dispatcher.
...
## Batch Mode — All States In Parallel

When the breeder requests a batch build, hand off to `cag-batch-rebuilder`, which:

1. Reads `data/locations.json` — all states where `"live": true`
2. Issues one `Agent` call per state in ONE message (`subagent_type: cag-location-builder`), each carrying:
```
...
7. **Batch mode requires explicit user approval** before dispatching all states at once
