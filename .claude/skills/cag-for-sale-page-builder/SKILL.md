...
- **In-body images** — uniform `.sec-img.inf-img` 760px 16:9 box for EVERY OG photo AND infographic (per-image `object-position`, <100 KB WebP + `-760.webp` sibling + srcset). **Every H2, H3, and key H4 gets an image** (OG or infographic/AI). Rule 50b alt rotation — no two images share an alt.
